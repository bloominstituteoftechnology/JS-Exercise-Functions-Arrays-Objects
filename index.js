...
*/
function getName(para1) {

  return "Hello, my name is " + para1.name;
}
...
*/
function appleIndex(myArr) {
var fruits = myArr;
  var app = fruits.indexOf('apple');
  return app;
}
...
*/
function isItAnApple(arrayBool) {
  const newArr = [];
  for ( let i = 0 ; i < arrayBool.length ; i++ ) {
    if (arrayBool[i] === 'apple'){
      newArr.push (true);
    }
    else{
      newArr.push (false);
...
function getLastCarInfo(carArr) {
  const carIn = carArr.length - 1;
  const theLast = carArr[carIn];
  return `The is a ${theLast.car_make} ${theLast.car_model}`
}
...
const yearofcars = []
  ...
  yearofcars.push(carInventory.car_year);
  }
return yearofcars;
}
