...
function getCarInfoByIndex(inventory, index) {
  return `This is a ${inventory[index].car_year} ${inventory[index].car_make} ${inventory[index].car_model}`
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inventory, invId) {
...
function sortCarInventory(inventory) {
  return inventory.sort((car1, car2) => car1.car_model < car2.car_model ? -1 : car1.car_model > car2.car_model ? 1 : 0)
}
...
function getOlderCars(inventory, maxYear) {
  return inventory.filter(year => year.car_year <= maxYear)
}
...
function getGermanCars(inventory) {
  const germanMakes = ["Mercedes-Benz", "BMW", "Volkswagen", "Audi"]
  const germanCars = inventory.filter(car => germanMakes.includes(car.car_make))
  return germanCars
}
