...
function getCarInfoById(inventory, id) {
  ...
  for ( let i = 0; i < inventory.length; i++ ) {
    // compare the inventory[i].id and find the object that matches id argument.
...
function sortCarInventory(inventory) {
  /* code here */
  const cars = inventory.sort(function (carone, cartwo) {
    if(carone.car_model < cartwo.car_model)
    {return -1}
    if(carone.car_model > cartwo.car_model)
    {return 1}
    else 
...
*/
function getOlderCars(inventory, endyear) {
  /* code here */
  let oldcars = [];
  let next = 0
  for (let i = 0; i < inventory.length; i ++) {
    if (endyear >= inventory[i].car_year)
    oldcars[next] = inventory[i].car_year;
    next ++
  }
  return oldcars;
}
