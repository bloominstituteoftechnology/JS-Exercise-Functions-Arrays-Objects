...
function getOlderCars(cars, maxYear) {
  let filteredCarByYears = [];
  for (let i = 0; i <= cars.length; i++) {
  	if (cars[i] && cars[i].car_year <= maxYear) filteredCarByYears[filteredCarByYears.length] = cars[i]
  }
  return filteredCarByYears;
}
...
function getGermanCars(cars) {
  const germanBranchs = ['Audi', 'Mercedes-Benz', 'Volkswagen', 'BMW'];
  let filteredGermanCars = [];
  for (let i = 0; i <= cars.length; i++) {
  	if (cars[i] && germanBranchs.indexOf(cars[i].car_make) != -1) filteredGermanCars[filteredGermanCars.length] = cars[i]
  }
  return filteredGermanCars;
}
 ...
 * 
 * @instructions
 * Create arrow function ver sions of the following commented-out functions:
 * 
