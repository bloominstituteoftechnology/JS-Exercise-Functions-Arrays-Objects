...
function sayGoodbye(name) {
  const string = `Goodbye, ${name}. Have a great day.`;
  return string;
}
...
*/
  function temperatureInF(temp, unit) {
    ...
    if (unit === "F") {
      ...
      const cel = temperatureCtoF(temp);
      const newTemp = `${cel}F`
      return newTemp;
    }
  }
...
function getName(name) {
  ...
  const greet = `Hello, my name is ${name.name}`;
  return greet;
}
...
function getCarInfoByIndex(inventory, index) {
  ...
  const mssg = `This is a ${car.car_make} ${car.car_model}`;
  return mssg;
}
...
function getLastCarInfo(inventory) {
  for(let i = 0; inventory.length < 1000000; i = i + 1) {
    const val = inventory[inventory.length - 1];
    const mssg = `This is a ${val.car_make} ${val.car_model}`;
    return mssg;
  }
