 ...
 * it will return `This is a Lincoln Navigator`.
 */
function getCarInfoById(carList, carId) {
  /* code here */

  // Loops through the carList
  for (let i = 0; i < carList.length; i++) {
    // Checks if id of the car matches. If it does, returns the car information
    if (carList[i].id === carId) {
      return `This is a ${carList[i].car_model} ${carList[i].car_make}`;
    }
  }
}
...
function sortCarInventory(carList) {
  ...
  return carList.sort(function(car1, car2) {
...
function getOlderCars(carList, maxYear) {
  ...
  // Loops through the carList array
  for (let i = 0; i < carList.length; i++) {
    // Checks if the car_year is lessthan the MaxYear
    if (carList[i].car_year <= maxYear) {
      olderCars.push(carList[i]);
    }
  }
...
function getGermanCars(carList) {
  ...
  // Assign an empty array that we can add cars too.
  let germanCars = [];

  // Loops through each car and checks if the car make is either Audi, Mercedes-Bens, Volkswagen or BMW and pushes them to the new array
  carList.forEach(car => {
    if (
      car.car_make === "Audi" ||
...
function carMaker(odometerReading) {
  ...
  const car = {
    ...
    drive: distance => {
      // Needs to declare the car.odometer, because it does not know what the latest odemeter reading is.
      // By calling the object.property. it will always look at the latest changes
